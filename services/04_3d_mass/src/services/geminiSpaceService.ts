...
2. 모든 층의 (netArea + commonArea) 총합은 반드시 목표 연면적의 95% ~ 100% 사이(${ (constraints.grossFloorArea * 0.95).toFixed(0) }㎡ ~ ${constraints.grossFloorArea.toFixed(0)}㎡) 가 되도록 전체 잉여 면적을 남김없이 분배하세요!!! (매우 중요: 현재 14,000㎡ 규모의 거대한 건물을 설계 중인데 800㎡ 수준으로 터무니없이 작게 만들지 마세요.)
...
7. 면적을 큰 덩어리 1, 2개로 짐작해서 분배하지 말고, 14,000㎡ 수준의 대형 프로젝트임을 감안하여 큰 숫자를 과감하게 사용하여 스케일에 맞게 분배하세요.
...
`;
        ...
        const parsed: AISpaceFloor[] = JSON.parse(content);
        return parsed;
