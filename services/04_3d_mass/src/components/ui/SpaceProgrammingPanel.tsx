...
import { Layers, Plus, Trash2, GripVertical, AlertTriangle, Send, Loader2, Wand2, Search, Compass, ChevronDown, Globe } from 'lucide-react';
...
export default function SpaceProgrammingPanel() {
    const {
        ...
        moveRoomToFloor, autoGenerateSpaceProgram,
        address, landArea, zoneType, buildingCoverageLimit, floorAreaRatioLimit,
        totalFloors, grossFloorArea
    } = useProjectStore();
    
    const store = useProjectStore(); // full store for address search
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50">
                ...
                <div className="p-6 flex flex-col xl:flex-row gap-6 max-w-[1400px] mx-auto w-full">
                    ...
                    <div className="w-full xl:w-1/2 space-y-6 flex flex-col shrink-0">
                        ...
                        {/* ────── 주소 검색 ────── */}
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 relative" style={{ zIndex: 100 }} ref={dropdownRef}>
                            <div className="flex items-center gap-2 mb-2">
                                <div className="w-6 h-6 rounded-md bg-blue-50 flex items-center justify-center shrink-0">
                                    <Search size={12} className="text-blue-600" />
                                </div>
                                <h3 className="text-[12px] font-bold text-slate-800">대상지 주소 검색</h3>
                            </div>
                            <div className="flex gap-2">
                                <div className="flex-1 relative">
                                    ...
                                    <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                                        ...
                                        className="w-full bg-white border border-slate-200 rounded-lg pl-8 pr-3 py-1.5 text-[11px] text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400/20 transition-all"
                                        placeholder="주소 입력 후 Enter" />
                                </div>
                                <button onClick={executeSearch} disabled={isSearching || !searchQuery.trim()}
                                    className="px-2.5 py-1.5 rounded-lg font-semibold text-[11px] text-white bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 disabled:opacity-40 transition-all flex items-center gap-1 shrink-0 shadow-md">
                                    {isSearching ? <Loader2 size={10} className="animate-spin" /> : <Search size={10} />} 검색
                                </button>
                            </div>
                            {/* 예시 필지 */}
                                                ...
                                                {MOCK_PARCELS.map(p => {
                                                    ...
                                                    return (
                                                        <button key={p.id} onClick={() => handleSelectParcel(p)}
                                                            className={`w-full text-left p-1.5 rounded-lg text-[10px] transition-all flex items-center gap-2 border ${isActive ? 'bg-blue-50 border-blue-300' : 'bg-slate-50 hover:bg-slate-100 border-slate-100'}`}>
                                                            <ParcelShape polygon={p.polygon} active={isActive} size={20} />
                            ...
                            <AnimatePresence>
                                {showDropdown && kakaoResults.length > 0 && (
                                    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                                        className="absolute left-4 right-4 bg-white border border-slate-200 rounded-xl overflow-hidden shadow-2xl max-h-[160px] overflow-y-auto"
                                        style={{ zIndex: 9999, top: '78px' }}>
                        ...
                        <div className="p-0">
                            <table className="w-full text-[11px] text-left">
                                <tbody className="divide-y divide-slate-100">
                                    ...
                                    <tr>
                                        <th className="py-3 px-4 bg-slate-50 text-slate-600 font-semibold">연면적</th>
                                        <td className="py-3 px-4 font-bold text-blue-600">{actualGFA.toLocaleString(undefined, { maximumFractionDigits: 2 })} ㎡</td>
                                    </tr>
                                    <tr>
