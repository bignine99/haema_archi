...
## 🚀 [2026-04-09] ARCHE 플랫폼 전역 안정화 및 AI 파싱 엔진 고도화 완성

### 2026-04-09 작업 내용
프로젝트 전역에 남아 있던 잔존 버그들을 해결하고, 정규식(Regex) 기반 데이터 추출의 근본적 한계를 극복하기 위해 Gemini 기반의 Full-AI 파싱 엔진으로 전면 업그레이드했습니다. 또한, 다중 배포 환경으로 인한 혼선을 방지하기 위해 단일 인프라로 병합했습니다.

### 1. 과업지시서 AI 파싱 엔진 전면 업그레이드 (Zero-Dropout Extraction)
기존에 `documentParser.ts` 단계에서 사업명, 대지위치, 연면적 및 건폐율 등의 수치를 추출할 때 사용되던 Regex(정규식) 패턴이 특수 포맷의 문서에서 지속적으로 오작동하는 한계를 발견하여 해결했습니다.
- **수정 방안**: Regex 로직을 단순 백업용으로 돌리고, `geminiService.ts`의 `ANALYSIS_PROMPT`를 확장하여 **AI가 문맥 전체를 읽고 사업명과 12대 핵심 프로젝트 제원을 직접 파싱 및 추출**하도록 아키텍처 고도화 수행.
- **결과**: 기출 과업지시서 PDF에서 포맷 변환 및 띄어쓰기 훼손이 발생해도 AI가 맥락상 사업명을 완벽히 인지하여 `ParsedProjectData`를 정확히 생성합니다.

### 2. 단일 인프라 통합 (Vercel 혼선 차단)
사용자의 피드백 ("두 개의 서버를 사용하는 것이 아니라 홈페이지 하나만 사용하는 것으로 통합하자")을 100% 반영 완료.
- **문제점**: Vercel Preview 배포(개발계)와 실제 Nginx 웹서버(`ninetynine99.co.kr/haema`) 라이브 간의 버전 스프롤(Sprawl) 현상.
- **해결책**: 혼동을 유발하던 Vercel 파이프라인을 완전 중단 및 폐기하고 `deploy_patch.bat` 스크립트를 통한 EC2 단일 배포 체제로 로드맵 통합.

### 3. 기능 공간(Space) 할당 논리 고도화 (Zero-Hallucination)
- **문제점**: "실배치를 보면 목욕실이 없다" 등 체육/다목적 시설임에도 불구하고 과거 프롬프트 오염으로 인해 고의 누락 또는 '학교 시설'로 환각을 일으키는 현상 발생.
- **해결책**: `geminiSpaceService.ts`의 Prompt에 **ZERO-DROPOUT 전략**을 신규 편입. AI가 임의로 면적을 합산해버리거나 필요 실(Room)을 버리지 않도록, RFP에 기입된 핵심 요구 실(목욕실, 사우나, 기계실 등)을 1:1로 매핑하여 JSON으로 출력하도록 강력한 제약 로직 구축.
