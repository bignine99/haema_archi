...
export default function SpecsAnalysisPanel() {
            ...
            <div className="h-full w-full flex flex-col bg-slate-50/50">
                ...
                <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-slate-200 px-8 py-5 flex items-center justify-between rounded-t-3xl z-20 shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-lg font-black text-slate-800 tracking-tight">AI 프로젝트 제원 분석</h3>
                            <p className="text-[11px] font-medium text-slate-500 mt-0.5">과업지시서 자동 추출 · 인증 · 지침 통합 대시보드</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className="w-2 h-2 rounded-full bg-slate-300"></span>
                        <span className="text-xs font-bold text-slate-400 tracking-widest">PARSER IDLE</span>
                    </div>
                </div>
                <div className="flex-1 flex flex-col items-center justify-center gap-5 p-8">
                    ...
                    <h2 className="text-lg font-bold text-slate-800">과업지시서 업로드 대기 중</h2>
                    ...
                    <span className="text-[10px] px-3 py-1.5 rounded-full bg-blue-50 text-blue-600 border border-blue-200 font-medium">
    ...
    const barColor = rate >= 80 ? 'bg-emerald-500' : rate >= 50 ? 'bg-amber-500' : 'bg-red-400';
        ...
        <div className="h-full w-full flex flex-col bg-slate-50/50">
            ...
            <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-slate-200 px-8 py-5 flex items-center justify-between rounded-t-3xl z-20 shrink-0">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 pl-3 border-l border-slate-200">
                        ...
                        <span className="text-xs font-bold text-emerald-600 tracking-widest">PARSER ACTIVE</span>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-12 gap-6">
                    ...
                    <div className="col-span-12 xl:col-span-8 flex flex-col gap-5">
                        ...
                        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                            ...
                            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl px-4 py-3 mb-3">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                <div className="bg-slate-50 rounded-xl px-3 py-2.5 col-span-1">
                    ...
                    <div className="col-span-12 xl:col-span-4 flex flex-col gap-5">
                        ...
                        {/* PARSER ENGINE 대시보드 */}
                        <div className="bg-gradient-to-b from-slate-800 to-slate-900 rounded-2xl p-5 text-white shadow-xl relative overflow-hidden border border-slate-700">
                            <div className="absolute -right-4 -top-4 opacity-10"><FileText size={120} /></div>
                            <div className="flex items-center gap-2 mb-1 relative z-10">
                                <span className="w-2 h-2 rounded-full bg-blue-400 animate-pulse"></span>
                                <span className="text-xs font-bold text-blue-400 tracking-widest">DOCUMENT PARSER</span>
                            </div>
                            <h3 className="text-xl font-bold mb-4 relative z-10">제원 추출 엔진</h3>

                            <div className="flex-1 space-y-3 relative z-10">
                                {[
                                    { label: 'Project Overview', desc: '사업명, 위치, 용도 자동 추출', active: store.projectName !== '미정 프로젝트', color: 'blue' },
                                    { label: 'Area & Scale', desc: '면적·층고·건폐율·용적률 파싱', active: store.landArea > 0, color: 'cyan' },
                                    { label: 'Cert. Analyzer', desc: '인증 요구사항 및 법규 항목 분석', active: store.certifications.length > 0, color: 'violet' },
                                    { label: 'Budget Parser', desc: '사업비 및 일정 데이터 추출', active: !!store.constructionCost, color: 'emerald' },
                                ].map((item, i) => (
                        ...
                        {/* 확인사항 위젯 */}
                        {store.keyNotes.length > 0 && (
                            <div className="bg-white rounded-2xl border border-rose-200 shadow-sm p-5 border-t-4 border-t-rose-400">
                                <div className="flex items-center gap-2 mb-3">
                                    <AlertCircle size={15} className="text-rose-500" />
                                    <h3 className="text-[13px] font-bold text-slate-800 flex-1">핵심 확인사항</h3>
                                    <span className="text-[9px] px-2 py-0.5 bg-rose-100 text-rose-600 rounded-full font-bold">{store.keyNotes.length}건</span>
                                </div>
                                <div className="space-y-2">
                                    {store.keyNotes.slice(0, 4).map((note, i) => (
                                        <div key={i} className="flex items-start gap-2 bg-rose-50 rounded-lg p-2 border border-rose-100">
                                            <span className="text-rose-500 font-black text-[10px] mt-0.5 shrink-0">{String(i + 1).padStart(2, '0')}.</span>
                                            <p className="text-[11px] text-slate-700 font-medium leading-snug">{note}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
            ...
            <div className="shrink-0 border-t border-slate-200 bg-white px-6 py-3 flex items-center justify-between z-10">
                <div className="flex items-center gap-2 pr-4 border-r border-slate-200">
                    ...
                    <div>
                        <div className="text-[11px] font-black text-slate-800 leading-none mb-1">SPECS ANALYSIS</div>
                        <div className="text-[8px] text-slate-500 uppercase font-bold tracking-widest">Document Intelligence</div>
                    </div>
                </div>
                <div className="flex-1 flex justify-evenly text-[10px] font-medium px-2 text-center">
                    <div className="flex flex-col items-center gap-0.5"><span className="text-slate-800 font-bold">사업명</span><span className="text-blue-600 text-[9px] font-black truncate max-w-[80px]">{store.projectName}</span></div>
                    <div className="w-[1px] h-6 bg-slate-200"></div>
                    <div className="flex flex-col items-center gap-0.5"><span className="text-slate-800 font-bold">대지면적</span><span className="text-slate-500 text-[9px]">{store.landArea ? store.landArea.toLocaleString() + ' ㎡' : '-'}</span></div>
                    <div className="w-[1px] h-6 bg-slate-200"></div>
                    <div className="flex flex-col items-center gap-0.5"><span className="text-slate-800 font-bold">연면적</span><span className="text-slate-500 text-[9px]">{store.grossFloorArea ? store.grossFloorArea.toLocaleString() + ' ㎡' : '-'}</span></div>
                    <div className="w-[1px] h-6 bg-slate-200"></div>
